...
class Policy(nn.Module):
    ...
    def forward(self, state):
        ...
        std = torch.clamp(std, min=-2, max=10).mean(axis=0)
...
class MetaTemplate(nn.Module):
    def __init__(
        ...
        self.change_way = (
            change_way  # some methods allow different_way classification during training and test
        )
    ...
    def train_loop(self, epoch, train_loader_ori, optimizer, total_it):
        ...
        for i, (x_ori, global_y) in enumerate(train_loader_ori):
            ...
            (
                ...
                scores_cls_adv,
                loss_cls_adv,
            ) = self.set_forward_loss_StyAdv(x_ori, global_y, epsilon_list)
            ...
            if (i + 1) % print_freq == 0:
                print(
                    "Epoch {:d} | Batch {:d}/{:d} | Loss {:f}".format(
                        epoch, i + 1, len(train_loader_ori), avg_loss / float(i + 1)
                    )
                )
            if (total_it + 1) % 10 == 0:
                logs = {
